# ═══════════════════════════════════════════════════════════
#  PACTUM UPDATE KIT — سكربت التطبيق الآلي (نسخة 5 — فيها حارس الأمان)
#
#  طريقة الاستخدام:
#  1) فك pactum-update-kit.zip
#  2) انسخ التلات ملفات (pactum-clean.zip + APPLY.ps1 + UPDATE_MESSAGE.txt)
#     جوه فولدر المشروع:  D:\Projects\pactum
#  3) كليك يمين على APPLY.ps1  ←  "Run with PowerShell"
#
#  الجديد: حارس أمان يمنع تشغيل السكربت في مكان غلط
# ═══════════════════════════════════════════════════════════
...
# ── حارس المكان: لازم نكون جوه فولدر مشروع Pactum حقيقي ──
if (-not ((Test-Path (Join-Path $proj 'src\App.tsx')) -and (Test-Path (Join-Path $proj '.git')))) {
  Write-Host "ERROR: هذا المجلد ليس مجلد مشروع Pactum!" -ForegroundColor Red
  Write-Host "السكربت ماتنفذش عشان حمايتك." -ForegroundColor Red
  Write-Host "المكان الصح: انسخ الملفات جوه  D:\Projects\pactum  وحاول تاني." -ForegroundColor Yellow
  Read-Host "Press Enter to close"; exit 1
}
Write-Host "Safety check passed: this is the Pactum project folder." -ForegroundColor Green
...
$mustExist = @('src\App.tsx','src\main.tsx','src\components\modules\BaselineModule.tsx','src\components\modules\ContractModule.tsx','tests\golden\evm.golden.test.ts','vitest.config.ts','package.json','README.md')
$missing = @()
