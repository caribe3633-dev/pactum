...
import { useCompany, useMasterDataVersion } from '../lib/useMasterData';
...
function HealthArc({ score }: { score: number }) {
  ...
  return (
    <svg viewBox="0 0 100 60" className="w-full max-w-[140px]" aria-label={`Company health score: ${score}`}>
...
// ══════════════════════════════════════════════════════════════════════
// Company Analytics — executive analytics for a SINGLE company
//
//   Projects  ->  Sector Dashboard  ->  Company Analytics
//
// Route: /company/:id/analytics
// ══════════════════════════════════════════════════════════════════════
export default function CompanyAnalyticsPage() {
  ...
  // ── Company scoping ──────────────────────────────────────────────────
  const [, params] = useRoute('/company/:id/analytics');
  const companyId = params?.id;
  const company = useCompany(companyId);

  /**
   * THE UNIT THIS PAGE REPORTS IN — the COMPANY's currency.
   *
   * Same defect as the sector page: 22 of 23 `fmtSAR()` calls printed a
   * hardcoded 'SAR' over totals that were summed across projects in
   * several different currencies without conversion.
   */
  const scopeCcy = useMemo(
    () => scopeCurrency('company', companyId ?? ''), [companyId],
  );
  const pageCcy = scopeCcy.currency;
  const money = useCallback((n: number) => fmtSAR(n, pageCcy), [pageCcy]);

  // Roll up from this company's sectors only
  const scopedProjects = React.useMemo(() => {
    if (!companyId) return projects;
    const sectors = findSectorsByCompany(companyId);
    return projects.filter(p => sectors.some(s => s.projectIds.includes(p.id)));
  }, [projects, companyId]);
  ...
  const calculate = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const computed = computePortfolioMetrics(
        ...
        // Convert every project into the COMPANY's currency before summing.
        { targetCurrency: pageCcy, companyId },
      );
  ...
  }, [scopedProjects, pageCcy, companyId]);
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-64">
        <div className="text-white/30 text-sm font-mono animate-pulse">
          {lang === 'ar' ? 'جاري التحليل...' : 'Computing company metrics…'}
        </div>
      </div>
    );
  }
          ...
          {!scopeCcy.set && (
            <div className="ds-card border-chart-5/30 bg-chart-5/[0.05] !py-3">
              <p className="text-(length:--t-second) text-chart-5">
                {lang === 'ar'
                  ? `لم تُحدَّد عملة تقارير لهذا الشركة — يُعرض مؤقتاً بعملة الشركة (${pageCcy}). حدّدها من إعدادات الشركة.`
                  : `No reporting currency is set for this company. Showing the company's (${pageCcy}) provisionally — set it explicitly to make this a stated decision.`}
              </p>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-(length:--t-label) uppercase tracking-[0.3em] text-primary/60 font-mono">
                {lang === 'ar' ? 'تحليلات تنفيذية' : 'Executive Analytics'}
              </span>
            </div>
            <h1 className="font-serif text-3xl md:text-4xl text-white mb-1">
              {company ? company.name : (lang === 'ar' ? 'تحليلات الشركة' : 'Company Analytics')}
            </h1>
      ...
      <div className="w-full px-6 xl:px-8 2xl:px-10 pb-24 pt-10 space-y-14">
        ...
        <section aria-label={lang === 'ar' ? 'مؤشرات الأداء التنفيذية' : 'Executive KPIs'}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-px bg-white/[0.04] mb-4">
            {[
              mk('Total Company Value',      'إجمالي قيمة الشركة',     money(m.totalRevisedContractValue), m.totalRevisedContractValue, 'totalRevisedContractValue', { icon: DollarSign, highlight: 'gold', sub: money(m.totalContractValue) + ' original' }),
              ...
              mk('Company Progress',         'تقدم الشركة',             fmtPct(m.portfolioProgress),          m.portfolioProgress,          'portfolioProgress',          { icon: TrendingUp }),
        ...
        <section aria-label={lang === 'ar' ? 'تحليل القيمة المكتسبة' : 'EVM Dashboard'}>
          <SH en="Company EVM" ar="القيمة المكتسبة للشركة" lang={lang} icon={BarChart2} />
          ...
          <div className="border border-white/[0.06] bg-black/10 p-5">
            <p className="text-(length:--t-label) uppercase tracking-wider text-white/45 mb-4">
              {lang === 'ar' ? 'منحنى S للشركة' : 'Company S-Curve (PV / EV / AC)'}
            </p>
        ...
        {/* ════ Company Health ══════════════════════════════════ */}
        <section aria-label={lang === 'ar' ? 'صحة الشركة' : 'Company Health'}>
          <SH en="Company Health" ar="صحة الشركة" lang={lang} icon={Shield} />
        ...
        {/* ════ Charts ═════════════════════════════════════════ */}
        <section aria-label={lang === 'ar' ? 'الرسوم البيانية' : 'Company Charts'}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="border border-white/[0.06] bg-black/10 p-5">
              <p className="text-(length:--t-label) uppercase tracking-wider text-white/45 mb-4">
                {lang === 'ar' ? 'التدفق النقدي للشركة' : 'Company Cash Flow'}
              </p>
                ...
                <tfoot>
                  <tr className="border-t border-primary/20 bg-primary/[0.03]">
                    <td className="px-4 py-2.5 text-(length:--t-label) uppercase tracking-wider text-primary/60 font-bold">
                      {lang === 'ar' ? 'الإجمالي' : 'Company Total'}
                    </td>
